...
function Presence({ presence }) {
  if (presence && presence.details) {
    return (
      <div className="hidden bg-cover bg-center lg:flex hover:-translate-y-1 text-white max-w-xs min-h-max rounded-lg place-content-center transition-all ease-in-out duration-500 no-repeat w-max bg-indigo-800 border-0 absolute left-5 bottom-5 overflow-clip" shadow hoverable bordered={false} style={{ backgroundImage: `url(${presence.image})` }}>
